...
def markov_chain(data_set, no_iteration=10, no_of_simulation=10000, alpha=5):
    ...
    import_dataset_v1 = (
        import_dataset_v1.reindex(import_dataset_v1.index.repeat(import_dataset_v1.conversions))).reset_index()
    ...
    for k in range(0, no_iteration):
        ...
        import_data = pd.DataFrame({'path': np.random.choice(import_dataset['path'],
                                                             ...
                                                             p=import_dataset['probability'], replace=True)})
    ...
    final_df.columns = ['channel', 'ass_conversion', 'p_value', 'confidence_status', 'frequency', 'standard_deviation',
                        't_statistics']
    final_df['ass_conversion'] = sum(import_dataset['conversions']) * final_df['ass_conversion'] / sum(
